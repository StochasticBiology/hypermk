...
# TO DO: label the plots
results.fig = function(combined.obj, flux.threshold.pmax = 0.01, branch.lengths = FALSE) {
  ...
  title.irrev = paste0("irreversible fit, simplified AIC ~ ", round(AIC.rev.reduced, digits=2), 
                     " (full ", round(AIC.rev, digits=2), ")", collapse = "")
  ...
  if(branch.lengths == FALSE) {
    g.data = combined.obj$dset$data.plot
  } else {
    g.data = combined.obj$dset$data.plot.nb
  }
  
  return( ggarrange(g.data, g.irrev, g.rev, nrow = 1, labels = c("A", "B", "C")) )
}
