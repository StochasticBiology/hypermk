## Run cross.sectional.cross, with a sample size of 100
...
setup.data = function(expt) {
  ...
  if(expt == "cross.sectional.single" |
    ...
    if(expt == "cross.sectional.cross") {
      ...
      ## Ensure at least one of each type, and the rest from sampling.
      m = m[c(seq_along(nrow(m)),
              sample(1:nrow(m), sample.size - nrow(m), replace = TRUE)), ]
    }
...
if (TRUE) {
  ...
  set.seed(1) ## repeatable, but different runs
  for (nrun in 1:10) {
    ...
    for(i in 1:nexpts) {
      ...
      png(paste0("expt-", these_expts[i], "-N-100_nrun_", nrun, "optim_single_trial.png"), width=1000*sf, height=350*sf, res=72*sf)
      print(fig.list[[i]])
      dev.off()
    }
  }
